  ...
  <div class="sub-page">
    <div class="g-card">
      <el-form ref="queryParams" :model="queryParams" inline>
        ...
        <el-form-item label="产品类型" prop="edgeCategory">
          <el-select v-model="queryParams.edgeCategory" placeholder="请选择产品类型">
            <el-option v-for="item in dict.edgeCategory" :label="item.label" :value="item.value" :key="item.value">
            </el-option>
          </el-select>
        </el-form-item>
...
export default {
  ...
  //挂载完成（可以访问DOM元素）
  mounted() {
    this.getDicts('edge_product_type').then((res) => {
      this.dict.edgeCategory = res.data.data
      this.getList()
    })
  },
  //方法集合
  methods: {
    ...
    formatType(edgeCategory) {
      return this.selectDictLabel(this.dict.edgeCategory, edgeCategory)
    },
    handleAdd() {
    ...
    handleDetails(row) {
      ...
      this.$router.push('/app/product/edge/details/index?id=' + id)
    },
  },
}
</script>
