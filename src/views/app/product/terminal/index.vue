    ...
    <div class="g-card">
      <el-form ref="queryParams" :model="queryParams" inline>
        ...
        <el-form-item label="产品类型" prop="deviceCategory">
          <el-select v-model="queryParams.deviceCategory" placeholder="请选择产品类型">
            <el-option v-for="item in dict.deviceCategory" :label="item.label" :value="item.value" :key="item.value">
            </el-option>
          </el-select>
        </el-form-item>
        ...
        <div class="arit-list" v-if="tableData && tableData.length > 0">
          <div class="cell" v-for="(item, index) in tableData" :key="index">
            ...
            <div class="tit">
              ...
              <span>
                {{ formatType(item.deviceCategory) }}
              </span>
            </div>
...
export default {
  ...
  mounted() {
    this.getDicts('equipment_type').then((res) => {
      this.dict.deviceCategory = res.data.data
      this.getList()
    })
  },
  //方法集合
  methods: {
    ...
    formatType(deviceCategory) {
      return this.selectDictLabel(this.dict.deviceCategory, deviceCategory)
    },
    handleAdd() {
    ...
    handleDetails(row) {
      ...
      this.$router.push('/app/product/terminal/details/index?id=' + id)
    },
  },
}
</script>
