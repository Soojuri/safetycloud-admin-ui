<template>
  <div class="sub-page">
    <div class="g-card">
      ...
      <el-table :data="tableData" v-loading="loading" border style="width: 100%" @selection-change="handleSelect">
        <el-table-column type="selection"></el-table-column>
        <el-table-column prop="ruleName" label="规则名称" width="200" show-overflow-tooltip></el-table-column>
      ...
      <excel-upload ref="excelUpload" tempName="重复规则导入模板.xlsx" :tempUrl="tempUrl" title="重复规则导入模板" :url="importUrl"
...
import { getReEventList, delReEvent, changeStatus, exportEvent } from '@/api/app/rules/reEvent'
import { getManageList, importEvent } from '@/api/app/algorithm/manage'
...
export default {
  ...
  data() {
    return {
      ...
      loading: false,
      selectList: [],
      tempUrl: '/admin/systemplate/getByName/重复规则导入模板',
      importUrl: process.env.VUE_APP_BASE_API + '/iefVasService/tvideoanalyseruleconfigrepeat/import',
    }
  },
  computed: {
  ...
  methods: {
    getList() {
      this.loading = true
      getReEventList(this.queryParams)
        .then((res) => {
          this.loading = false
    ...
    handleAdd() {
      if (!this.permissions.rules_reEvent_add) return this.msgWarn('权限不足')
      this.formOptions.visible = true
      this.formOptions.data = {}
    },
    handleEdit(row) {
      if (!this.permissions.rules_reEvent_edit) return this.msgWarn('权限不足')
    ...
    handleDelete(row) {
      if (!this.permissions.rules_reEvent_delete) return this.msgWarn('权限不足')
      const that = this
      this.$confirm('是否确认删除该重复规则', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning',
      })
        .then(() => {
          return delReEvent(row.ruleId)
        })
        .then((res) => {
          if (res.data.data) {
    ...
    handleChangeStatus(row) {
      if (!this.permissions.rules_reEvent_start) return this.msgWarn('权限不足')
      this.loading = true
    ...
    handleImport() {
      if (!this.permissions.rules_reEvent_Pin) return this.msgWarn('权限不足')
      this.$refs.excelUpload.show()
    },
    handleExport() {
      if (!this.permissions.rules_reEvent_Pout) return this.msgWarn('权限不足')
