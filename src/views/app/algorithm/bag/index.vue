        ...
        <div class="arit-list" v-if="tableData && tableData.length > 0">
          <div class="cell" v-for="(item, index) in tableData" :key="index">
            ...
            <div class="tit">
              ...
              <span>
                {{ item.algorithmType == 1? '通用算法类' :item.algorithmType == 2? '城市治理类' :item.algorithmType == 3? '应急管理类' :'水域治理类' }}
              </span>
...
<script>
import PopForm from '@/views/app/algorithm/bag/popFrom.vue'
import { getAlgorithmList, delAlgorithm } from '@/api/app/algorithm/bag.js'
...
export default {
  ...
  methods: {
    ...
    handleEdit(row) {
      ...
      this.formOptions.data.id = row.packageId
      this.formOptions.visible = true
    },
    handleQuery() {
      const that = this
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除该算法分类', '提示', {
        ...
        .then(() => {
          return delAlgorithm(row.packageId)
        })
        .then((res) => {
          if (res.data.data) {
    ...
    handleDetails(row) {
      ...
      const id = row.packageId
      this.$router.push('/app/algorithm/bag/details/index?id=' + id)
    },
  },
}
</script>
