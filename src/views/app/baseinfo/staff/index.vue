      ...
      <div class="mt-m">
        ...
        <div class="g-table">
          <el-table v-loading="loading" border :data="tableData">
            ...
            <el-table-column prop="protocolType" align='center' label="服务调用次数" :formatter="formatType" />
...
import { getStaffInfoList, delStaffInfo, putStaffInfo } from '@/api/app/baseinfo/staff.js'
...
export default {
  ...
  methods: {
    getList() {
      this.loading = true
      getStaffInfoList(this.queryParams)
        .then((res) => {
          this.loading = false
    ...
    handleEdit(row) {
      ...
      this.formOptions.data.id = row.staffId
      this.formOptions.visible = true
    },
    handleQuery() {
      const that = this
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除', '提示', {
        ...
        .then(() => {
          return delStaffInfo(row.staffId)
        })
        .then((res) => {
          if (res.data.data) {
    ...
    handleChangeStatus(row) {
      ...
      putStaffInfo({ staffId, status }).then((res) => {
        if (res.data.data) {
