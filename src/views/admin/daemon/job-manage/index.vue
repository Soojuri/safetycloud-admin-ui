<template>
  <div class="jobIndex">
    <basic-container>
      <avue-crud ref="crud" :page.sync="page" v-model="form" :data="tableData" :table-loading="tableLoading" :option="tableOption" @on-load="getList"
                 @row-save="save" @row-update="update" @refresh-change="refreshChange" @size-change="sizeChange" @current-change="currentChange"
                 @search-change="handleFilter" @search-reset="handleSearchReset">
        ...
        <template slot="menu" slot-scope="scope">
          <el-button type="text" icon="el-icon-info" @click="handleJobLog(scope.row)">日志
          </el-button>
          <el-button v-if="permissions.job_sys_job_start_job" type="text" icon="el-icon-caret-right" @click="handleStartJob(scope.row)">启动
          </el-button>

          <el-button v-if="permissions.job_sys_job_shutdown_job" type="text" icon="el-icon-error" @click="handleShutDownJob(scope.row)">暂停
          </el-button>
          <el-button v-if="permissions.job_sys_job_edit" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row,scope.index)">修改
          </el-button>
          <el-button v-if="permissions.job_sys_job_run_job" type="text" icon="el-icon-s-promotion" @click="handleRunJob(scope.row)">执行
          </el-button>
          <el-button v-if="permissions.job_sys_job_del" type="text" icon="el-icon-delete" @click="handleDelete(scope.row,scope.index)">删除
          </el-button>
        </template>
      </avue-crud>
    </basic-container>
  ...
  methods: {
    ...
    save (row, done, loading) {
      isValidTaskName({
        ...
        if (result !== 0) {
          this.msgError('任务名称与任务组重复，请确认后重新添加')
        } else {
          addObj(row).then(() => {
    ...
    handleDelete (row) {
      ...
      if (jobStatus === '1' || jobStatus === '3') {
        ...
        this.msgError('运行中定时任务不可删除，请先暂停后操作')
      }
    },
    handleJobLog (row) {
