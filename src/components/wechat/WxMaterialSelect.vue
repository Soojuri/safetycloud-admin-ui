...
<template>
  <div v-if="objData.repType == 'image'">
    <div class="waterfall" v-loading="tableLoading">
      <div class="waterfall-item" v-for="item in tableData" :key="item.mediaId">
        ...
        <el-row class="ope-row">
          <el-button size="mini" type="success" @click="selectMaterial(item,objData.appId)">选择<i class="el-icon-circle-check el-icon--right"></i>
          </el-button>
        </el-row>
      </div>
    </div>
    ...
    <span slot="footer" class="dialog-footer">
      <el-pagination @size-change="sizeChange" @current-change="currentChange" :current-page.sync="page.currentPage" :page-sizes="[10, 20]"
                     :page-size="page.pageSize" layout="total, sizes, prev, pager, next, jumper" :total="page.total" class="pagination">
      </el-pagination>
    </span>
  </div>
  <div v-else-if="objData.repType == 'voice'">
    <avue-crud ref="crud" :page.sync="page" :data="tableData" :table-loading="tableLoading" :option="tableOptionVoice" @on-load="getPage"
               @size-change="sizeChange" @refresh-change="refreshChange">
      <template slot-scope="scope" slot="menu">
        <el-button type="text" icon="el-icon-circle-plus" size="small" plain @click="selectMaterial(scope.row,objData.appId)">选择</el-button>
      </template>
    </avue-crud>
  </div>
  <div v-else-if="objData.repType == 'video'">
    <avue-crud ref="crud" :page.sync="page" :data="tableData" :table-loading="tableLoading" :option="tableOptionVideo" @on-load="getPage"
               @size-change="sizeChange" @refresh-change="refreshChange">
      <template slot-scope="scope" slot="menu">
        <el-button type="text" icon="el-icon-circle-plus" size="small" plain @click="selectMaterial(scope.row,objData.appId)">选择</el-button>
      </template>
  ...
  <div v-else-if="objData.repType == 'news'">
    ...
    <span slot="footer" class="dialog-footer">
      <el-pagination @size-change="sizeChange" :current-page.sync="page.currentPage" :page-sizes="[10, 20]" :page-size="page.pageSize"
