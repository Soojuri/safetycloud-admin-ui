...
<template>
  <div>
    <div v-if="!outTime"><i class="icon-shipinbofang" style="font-size: 80px!important;" @click="playVideo()"></i>
      <p>点击播放视频</p>
    </div>
    <div v-if="outTime">
...
export default {
  ...
  created() {
    this.outTime =
      this.objData.type === '1' &&
      parseInt(new Date().getTime() - new Date(this.objData.createTime).getTime()) >= 259200000
  },
  mounted() {},
  computed: {},
  methods: {
    ...
    getVideo() {
      if (this.objData.type == '2') {
        ...
        this.getTempMaterialOther(this.objData.repMediaId, this.objData.repMediaId + '.mp4').then((val) => {
