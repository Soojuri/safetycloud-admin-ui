      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <div class="row" v-if="ptz">
            <div :class="['form-group', 'play-area']" @mousemove="doActive">
              <LivePlayer ref="player" v-if="bShow" :videoUrl="videoUrl" :poster="poster" :live="live" muted
                          :hideBigPlayButton="!!serverInfo.HideBigPlayButton" @message="$message"
                          :loading.sync="bLoading" v-loading="bLoading" element-loading-text="加载中">
              </LivePlayer>
            </div>
            <div :class="['col-sm-4', 'form-group']">
              ...
              <div class="form-group">
                <div class="ptz-block">
                  ...
                  <div class="ptz-cell ptz-up" command="up" title="上" @mousedown.prevent="ptzControl('up', $event)"
                       @touchstart.prevent="ptzControl('up', $event)">
                    <i class="fa fa-chevron-up"></i>
                  </div>
                  <div class="ptz-cell ptz-left" command="left" title="左"
                       @mousedown.prevent="ptzControl('left', $event)" @touchstart.prevent="ptzControl('left', $event)">
                    <i class="fa fa-chevron-left"></i>
                  </div>
                  <div class="ptz-cell ptz-center" title="云台控制">
                    <i class="fa fa-microphone" title="按住喊话" @mousedown.prevent="talkStart"
                       @touchstart.prevent="talkStart" v-if="canTalk()"></i>
                    <i class="fa fa-microphone-slash" title="由于浏览器安全策略, 非 HTTPS 或 localhost 访问, 对讲不可用"
                       v-if="!canTalk() "></i>
                  </div>
                  <div class="ptz-cell ptz-right" command="right" title="右"
                       @mousedown.prevent="ptzControl('right', $event)"
                       @touchstart.prevent="ptzControl('right', $event)">
                    <i class="fa fa-chevron-right"></i>
                  </div>
                  <div class="ptz-cell ptz-down" command="down" title="下"
                       @mousedown.prevent="ptzControl('down', $event)" @touchstart.prevent="ptzControl('down', $event)">
                    <i class="fa fa-chevron-down"></i>
                  </div>
                  <div class="ptz-cell ptz-zoomin" command="zoomin" title="放大"
                       @mousedown.prevent="ptzControl('zoomin', $event)"
                       @touchstart.prevent="ptzControl('zoomin', $event)">
                    <i class="fa fa-plus"></i>
                  </div>
                  <div class="ptz-cell ptz-zoomout" command="zoomout" title="缩小"
                       @mousedown.prevent="ptzControl('zoomout', $event)"
                       @touchstart.prevent="ptzControl('zoomout', $event)">
                    <i class="fa fa-minus"></i>
                  </div>
                </div>
                <div class="clearfix"></div>
                <div class="row stream-info hidden-xs hidden-sm"
                     v-if="SourceVideoCodecName && SourceVideoWidth && SourceVideoHeight">
                  <div class="col-sm-3">
                    分辨率:
                  </div>
                  <div class="col-sm-3">
        ...
        <div class="modal-footer">
          <el-radio-group v-model.trim="protocol" size="small" @change="setProtocol" id="protocol-switcher"
          ...
          <button type="button" :class="['btn', {'btn-primary': !bRecording, 'btn-danger': bRecording}]"
